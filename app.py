...
def calculators(input):
    ...
    finalized_data = [0]*30 # Initialize an array of zeros (30 features total)

    # Min and max values for each of the 30 features the model expects
    features_ranges = {
        0 : {'min': 6.98, 'max':28.1},
        1 : {'min': 9.71, 'max':39.3},
        2 : {'min': 43.8, 'max':189},
    ...
    # Maps each question to the corresponding model feature indices
    questions_indices = {
        1 : [0,2,3,20,22,23],
        2 : [1, 4 , 21,24 ],
        3 : [5,25],
    ...
    return finalized_data #return the array with the right scale
...
def predict():
    try:
        ...
        # Prediction time (M or B)
        prediction = model.predict(input_data)[0]
        
        # Return the result to the frontend
        return render_template('index.html', prediction = prediction)
    
    # If anything wrong occur, show this error
